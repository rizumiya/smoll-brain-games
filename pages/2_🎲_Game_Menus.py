...
sidebar_btn_new_games = st.sidebar.button("New Game", key="sd_new_game_button")
...
if ('is_logged_in' in st.session_state
    ...
    if selected_game_type.lower() == 'number':
        ...
        if 'game_history_id' not in st.session_state:
            fields = ('game_id', 'game_level_id')
            values = ("1", "2")
            if 'guest' not in st.session_state:
                fields = ('player_id', 'game_id', 'game_level_id')
                values = (str(st.session_state["user_id"]), "1", "1")
            st.session_state['game_history_id'] = create_new_game(fields, values)[0][0]
            ...
            if not st.session_state['game_over']:
                ...
                if guess:
                    ...
                    if guess.isdigit() and len(guess) == 4 and len(set(guess)) == 4:
                        correct_numbers = sum(
                            1 for g in guess if g in secret_number)
                        correct_positions = sum(1 for x, y in zip(
                            guess, secret_number) if x == y)
                        st.error(
                            f"{correct_numbers} angka benar, {correct_positions} posisi benar.")
                        
                        # Menyimpan tebakan ke database
                        save_history_guessing(
                            st.session_state['game_history_id'], st.session_state['user_id'], guess, correct_numbers, correct_positions)

                        if correct_positions == 4:
                            st.session_state['game_over'] = True
                            if 'guest' not in st.session_state:
                                message = f"Selamat! Anda menebak dengan benar setelah {get_total_guessing_in_one_game(st.session_state['game_history_id'], st.session_state['user_id'])[0][0]} kali tebakan."
                            else:
                                message = "Selamat! Anda berhasil menebak angkanya dengan benar.."
                            st.success(message)
                        ...
                        st.error(
                            "Masukkan harus berupa 4 digit angka yang unik dan tidak berulang.")
            else:
                st.success(
                    "Permainan telah selesai. Mulai ulang untuk bermain lagi.")
                new_game = st.button("New Game")
                if new_game:
                    clear_text_input()
                    new_games()
        ...
        with col2:
            ...
            if 'guest' in st.session_state:
                rows = get_guessing_history_current_game(st.session_state['game_history_id'])
            else:
